...
class AzureStorageTest(TestCase):
    ...
    def test_open_stream(self):
        """Normalize Azure paths and read the byte interval returned by Azure."""

        data = b"0123456789"
        downloader = mock.MagicMock()
        downloader.chunks.return_value = (data[1:4], data[4:6])
        ...
        with self.storage.open_stream(
            ...
            self.assertEqual(stream.read(2), b"12")
            self.assertEqual(stream.read(2), b"34")
            self.assertEqual(stream.read(2), b"5")

        self.storage._client.download_blob.assert_called_once_with(
            "media/artifact", offset=1, length=5, timeout=self.storage.timeout
        )

    def test_open_stream_limits_an_overlong_provider_response(self):
        """Prevent an overlong Azure response from exceeding the selected range."""

        data = b"0123456789"
        downloader = mock.MagicMock()
        downloader.chunks.return_value = (data[1:4], data[4:7])
        self.storage._client.download_blob.return_value = downloader

        with self.storage.open_stream("artifact", start=1, length=5) as stream:
            self.assertEqual(stream.read(8), data[1:6])
    ...
    def test_open_stream_to_end(self):
        ...
        data = b"0123456789"
        downloader = mock.MagicMock()
        downloader.chunks.return_value = (data[3:6], data[6:])
        self.storage._client.download_blob.return_value = downloader

        with self.storage.open_stream("artifact", start=3) as stream:
            self.assertEqual(stream.read(3), b"345")
